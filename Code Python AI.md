...
lan_send = 0
classId_cu = 0
path="pic2\classroom1\classroom2"
images = []
classNames = []
classId = []
ma_id = 0
...
while True:
    ...
    for encodeFace, faceLoc in zip(encodecurFrame,facecurFrame): # lấy từng khuôn mặt và vị trí khuôn mặt hiện tại theo cặp
        ...
        if faceDis[matchIndex] <0.50 :
            ...
            print(classId[matchIndex])
            if lan_send == 0:
                conn.sendall(str(classId[matchIndex]).encode())
                classId_cu = classId[matchIndex]
                lan_send = 1
            if lan_send >= 1:
                if classId_cu != classId[matchIndex]:
                    conn.sendall(str(classId[matchIndex]).encode())
                    classId_cu = classId[matchIndex]
        else:
            name = "Unknow"
